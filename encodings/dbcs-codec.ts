...
class DBCSEncoder {
    ...
    write (str: string): ArrayBuffer {
        let newBuf = new ArrayBuffer(str.length * (this.gb18030 ? 4 : 3)),
            ...
            if (seqObj !== undefined && uCode != UNASSIGNED) { // We are in the middle of the sequence
                ...
                if (dbcsCode == UNASSIGNED && this.gb18030) {
                    ...
                    if (idx != -1) {
                        ...
                        newBufView[j++] = 0x81 + Math.floor(dbcsCode / 12600); dbcsCode = dbcsCode % 12600;
                        newBufView[j++] = 0x30 + Math.floor(dbcsCode / 1260); dbcsCode = dbcsCode % 1260;
                        newBufView[j++] = 0x81 + Math.floor(dbcsCode / 10); dbcsCode = dbcsCode % 10;
                        newBufView[j++] = 0x30 + dbcsCode;
                        continue;
                    }
                }
            }
